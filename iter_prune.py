...
def execute(
        ...
        batnorm=True,
        l1reg=True,
        model_set:Tuple[int,int,int,int,int]=(32,12,8,6,4),
        efficiency_calc=False,
        kfolds:int=4,
        fold:int=None,
    ):
    ...
    if batnorm:
        models = {32: models.three_layer_model_batnorm_masked(prune_mask_set[0], bn_affine=bn_affine, bn_stats=bn_stats), #32b
                  12: models.three_layer_model_bv_batnorm_masked(prune_mask_set[1],12, bn_affine=bn_affine, bn_stats=bn_stats), #12b
                  8: models.three_layer_model_bv_batnorm_masked(prune_mask_set[2],8, bn_affine=bn_affine, bn_stats=bn_stats), #8b
                  6:  models.three_layer_model_bv_batnorm_masked(prune_mask_set[3],6, bn_affine=bn_affine, bn_stats=bn_stats), #6b
                  4: models.three_layer_model_bv_batnorm_masked(prune_mask_set[4],4, bn_affine=bn_affine, bn_stats=bn_stats) #4b
                  }
    else:
    ...
    model_set = [models[m] for m in model_set]
    ...
    if fold is None: #No fold passed, just load a whole folder and randomly split train/test
        train_split = 0.75

        # Setup and split dataset
        full_dataset = jet_dataset.ParticleJetDataset(input_file, yamlConfig)
        ...
        train_dataset = jet_dataset.ParticleJetDataset(input_file, yamlConfig, filenames=train_filenames)
        print("K Fold Val Dataset:")
        val_dataset = jet_dataset.ParticleJetDataset(input_file, yamlConfig, filenames=[val_filename])
    ...
    test_dataset = jet_dataset.ParticleJetDataset(test, yamlConfig)
    ...
    for model, prune_mask, init_sd in zip(model_set, prune_mask_set, inital_models_sd):
        ...
        for prune_value in prune_value_set:
            ...
            if efficiency_calc and epoch_counter == 0:  # Get efficiency of un-initalized model
                aiq_dict, aiq_time = calc_AiQ(model,test_loader,batnorm=batnorm,device=device)
            ...
            if lottery:  # If using lottery ticket method, reset all weights to first initalized vals
            ...
            for epoch in range(epochs):  # loop over the dataset multiple times
                epoch_counter += 1
                # Train
                model, train_losses = train(model, optimizer, criterion, train_loader, L1_factor=L1_factor, l1reg=l1reg, device=device)
    ...
    filename = 'model_losses_{}.json'.format("_".join(model_set))
    ...
    # Plot metrics
    plot_total_loss(model_set, model_totalloss_set, model_estop_set, outputDir=output_dir)
    plot_total_eff(model_set,model_eff_set,model_estop_set, outputDir=output_dir)
...
if __name__ == "__main__":
    ...
    model_set = tuple(int(m) for m in options.model_set.split(','))
    ...
    execute(
        ...
        bn_affine=options.bn_affine,
        bn_stats=options.bn_stats,
        l1reg=options.l1reg,
        model_set=model_set,
        batnorm=options.batnorm,
        efficiency_calc=options.efficiency_calc,
        fold=options.fold,
